...
// 时柱通过 Lunar.fromYmdHms 内部计算，无需手动映射
...
export function calculateBaZi(
  ...
  if (isLunar) {
    // 农历日期直接创建 Lunar 对象，并传入时辰
    lunar = Lunar.fromYmdHms(year, month, day, hour, 0, 0);
  } else {
    // 公历转农历，需要保留时辰信息
    const solar = Solar.fromYmd(year, month, day);
    const lunarFromSolar = solar.getLunar();
    lunar = Lunar.fromYmdHms(lunarFromSolar.getYear(), lunarFromSolar.getMonth(), lunarFromSolar.getDay(), hour, 0, 0);
  }
